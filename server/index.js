...
import SocketDropServer from "./server.js";
import SocketDropWsServer from "./ws-server.js";
...
if (conf.signalingServer) {
    ...
    if (!isValidUrl || containsProtocol) {
        console.error("SIGNALING_SERVER must be a valid url without the protocol prefix.\n" +
            "Examples of valid values: `socketdrop.net`, `socketdrop.example.com:3000`, `example.com/socketdrop`");
        process.exit(1);
    }
...
const socketDropServer = new SocketDropServer(conf);

if (!conf.signalingServer) {
    new SocketDropWsServer(socketDropServer.server, conf);
} else {
...
console.log('\nSocketDrop is running on port', conf.port);
