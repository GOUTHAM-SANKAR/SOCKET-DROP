...
const cacheTitle = `Socketdrop-cache-${cacheVersion}`;
const relativePathsToCache = [
    ...
    'fonts/OpenSans/OpenSans-VariableFont_wdth,wght.ttf',
    'lang/en.json',
    'lang/ml.json'
];
...
const evaluateRequestData = function (request) {
    return new Promise(async (resolve) => {
        ...
        const SocketdropUrl = request.url;
        ...
        if (files && files.length > 0) {
            ...
            const DBOpenRequest = indexedDB.open('Socketdrop_store');
            DBOpenRequest.onsuccess = e => {
                ...
                for (let i = 0; i < fileObjects.length; i++) {
                    ...
                    objectStoreRequest.onsuccess = _ => {
                        if (i === fileObjects.length - 1) resolve(SocketdropUrl + '?share_target=files');
                    }
                }
            }
            DBOpenRequest.onerror = _ => {
                resolve(SocketdropUrl);
            }
        }
        else {
            ...
            resolve(SocketdropUrl + urlArgument);
        }
    });
}
