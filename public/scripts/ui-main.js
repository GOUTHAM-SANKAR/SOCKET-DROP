...
class ThemeUI {
    ...
    constructor() {
        ...
        this.$themeOceanBtn = document.getElementById('theme-ocean');
        this.$themeForestBtn = document.getElementById('theme-forest');
        this.$themeSunsetBtn = document.getElementById('theme-sunset');
        this.$themePurpleBtn = document.getElementById('theme-purple');
        this.$themeRoseBtn = document.getElementById('theme-rose');
        this.$themeMintBtn = document.getElementById('theme-mint');
        this.$themeCrimsonBtn = document.getElementById('theme-crimson');
        this.$themeGoldBtn = document.getElementById('theme-gold');

        this.themeButtons = {
            'auto': this.$themeAutoBtn,
            'light': this.$themeLightBtn,
            'dark': this.$themeDarkBtn,
            'ocean': this.$themeOceanBtn,
            'forest': this.$themeForestBtn,
            'sunset': this.$themeSunsetBtn,
            'purple': this.$themePurpleBtn,
            'rose': this.$themeRoseBtn,
            'mint': this.$themeMintBtn,
            'crimson': this.$themeCrimsonBtn,
            'gold': this.$themeGoldBtn
        };

        let currentTheme = this.getCurrentTheme();
        this.setTheme(currentTheme || 'auto');

        // Add event listeners for all theme buttons
        Object.keys(this.themeButtons).forEach(theme => {
            this.themeButtons[theme].addEventListener('click', () => this.onThemeClick(theme));
        });
    }
    ...
    setTheme(theme) {
        ...
        document.body.classList.remove('light-theme', 'dark-theme', 'ocean-theme', 'forest-theme', 
            'sunset-theme', 'purple-theme', 'rose-theme', 'mint-theme', 'crimson-theme', 'gold-theme');
        
        // Remove selected class from all buttons
        Object.values(this.themeButtons).forEach(btn => btn.classList.remove('selected'));
        ...
        if (theme === 'auto') {
            ...
            this.setCurrentTheme('auto');
            this.$themeAutoBtn.classList.add('selected');
        } else {
            // Apply specific theme
            document.body.classList.add(theme + '-theme');
            this.setCurrentTheme(theme);
            this.themeButtons[theme].classList.add('selected');
        }
    }

    // Legacy methods for backward compatibility
    onClickAuto() {
        this.onThemeClick('auto');
    }

    onClickLight() {
        this.onThemeClick('light');
    }

    onClickDark() {
        this.onThemeClick('dark');
    }
